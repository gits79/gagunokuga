...
- WebSocket 토이 만들기
#### 배운 것
- websocket 구현할 때 server와 client 따로 설정을 해줘야 한다.

## 🙇‍♀️ 사죄의 말씀 🙇‍♀️
- 2주차 금요일 ~ 3주차 금요일 설날 이슈로 업로드 못했습니다 ㅠㅠㅠ
...
- 처음으로 JPA를 사용해 봤다.
    ...
    >
    > `Pageable` : 페이지네이션을 위해 JPA에서 제공하는 구현체

- 원격 레포지토리의 브랜치들이 로컬 레포지토리에 안보일 때
    >`git remote update`
- practice1 브랜치를 현재 있는 브랜치와 병합하고 싶을 때
    > `git merge practice1`
#### 회고
- websocket으로 가구 배치를 하는 로직을 오랫동안 구현하려고 했는데, 결국 실패했다.
- 다행히 정호가 구현에 성공해서 정호 코드를 보고 공부해야겠다고 다짐했다.

### 🟠 25/02/04(Tue)
#### 오늘 한 일
- (BE) 이전 채팅 목록 불러오기
- (BE) websocket으로 실시간 채팅 구현 및 DB에 저장하기
#### 배운 것
- Controller에서 WebSocket 구현하기
    - `@MessageMapping("/{roomId}")`: 여기로 전송되면 메서드 호출 -> WebSocketConfig prefixes 에서 적용한건 앞에 생략
    - `@SendTo("/sub/{roomId}")` : 구독하고 있는 장소로 메시지 전송 (목적지)  -> WebSocketConfig Broker 에서 적용한건 앞에 붙여줘야됨
- `Chat` entity에서 `JoinColumn(name=user_id)` 설정해줬을 때
    - `userId`는 `User`와 연관된 관계로 설정되어 있기 때문에, `userId`를 `User` 객체로 매핑하여 저장해야 한다.
    - userRepository에서 user 정보 찾기
    ...
    ```
#### 궁금한 점
- DTO로 ChatResponse를 만들었는데, userId와 같은 pk 정보를 들고 다녀도 되는 것인지?
